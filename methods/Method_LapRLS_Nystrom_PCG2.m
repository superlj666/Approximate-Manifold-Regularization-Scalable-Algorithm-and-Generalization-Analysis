function [r_mse, iter, time, all_time, r_mse_arr] = Method_LapRLS_Nystrom_PCG2(K_ns, K_ms, K_ss, K_ts, L, vectors, options)    
    all_time=tic();
    ...
    P=K_ms'*K_ms+options.lambda_A*K_ss+(1e-6)*eye(length(sample_idx));
    function y=afun(x) 
    ...
    if options.performance
        ...
        time=toc(start);
        disp(['direct nystrom pcg using: ', num2str(time)]);    
        ...
        all_time=toc;
        disp(['direct nystrom pcg totally using: ', num2str(all_time)]);
    else
        % matlab pcg
        start=tic();
        [alpha, flag, rev, iter]=pcg(@afun,b,tol,maxit,@mfun);
        time=toc(start);
        disp(['direct nystrom pcg using: ', num2str(time)]);
        y_pre=K_ts*alpha;
        r_mse=sqrt(sum((y_pre-y_test).^2)/length(y_test));
        all_time=toc(all_time);
        disp(['direct nystrom pcg totally using: ', num2str(all_time)]);
        r_mse_arr=[];
    end
end