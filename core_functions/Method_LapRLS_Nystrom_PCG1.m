function [r_mse, iter, time, all_time, r_mse_arr] = Method_LapRLS_Nystrom_PCG1(K_ns, K_ms, K_ss, K_ts, L, vectors, options)    
    all_time=tic();
    ...
    if options.performance
        ...
        time=toc(start);
        disp(['nystrom pcg using: ', num2str(time)]);    
        ...
        all_time=toc;
        disp(['nystrom pcg totally using: ', num2str(all_time)]);
    else
        % matlab pcg
        start=tic();
